...
fn main() -> std::io::Result<()> {
    ...
    // 建立保存 conn_hash 的表格
    let mut conn_hash_table: Vec<ConnHashInfo> = Vec::new();

    // 初始化 conn_hash
    let mut conn_info = ConnHashInfo {
        conn_hash: format!("conn_{}", rng.gen::<u64>()),
        current_entries: 0,
        stmt_id: rng.gen_range(1..=30),
        exec_id: rng.gen_range(1..=100),
    ...
    for _ in 0..num_entries {
        // 每生成 10000 筆資料，輸出進度
        if conn_info.current_entries % 10000 == 0 {
            eprint!("\rGenerated {} entries...", conn_info.current_entries);
        }

        // 檢查是否需要生成新的 conn_hash
        if conn_info.current_entries >= 3000 || conn_info.exec_time >= Local::now() {
            // 保存舊的 conn_hash 到表格
            conn_hash_table.push(conn_info);

            // 生成新的 conn_hash
            conn_info = ConnHashInfo {
                conn_hash: format!("conn_{}", rng.gen::<u64>()),
                current_entries: 0,
                stmt_id: rng.gen_range(1..=30),
                exec_id: rng.gen_range(1..=100),
        ...
        // 更新 exec_time，增加 1 到 30 秒
        let increment_seconds = rng.gen_range(1..=30);
        conn_info.exec_time = conn_info.exec_time + Duration::seconds(increment_seconds as i64);
